...
> 2026-07-27 개정 — 등록은 관리자 전용, 노출은 담기(구독) 기준.
> 규칙 전문과 배경: [content-governance.md](content-governance.md)

| 구분 | 등록 위치 | visibility | 추출 항목 편입 | 노출 범위 |
|------|-----------|------------|----------------|-----------|
| 공용 | 백오피스 (admin) | `public` | 관리자 검수(승인) 후 | **담은 사용자** |
| 개인 | (신규 등록 차단, 잔존분만) | `private` | 자동 편입 (검수 없음) | 담은 사용자 |

- 항목(learning_items)은 전역 dedup 유지. 노출 규칙: "내가 담은 공용 콘텐츠의 승인 항목" ∪ "내가 담은 개인 콘텐츠의 항목(rejected 제외)". 단일 지점은 `visible_item_clause`.
- 개인 항목 제외는 검수 상태가 아니라 **본인 카드 suspend** 로 처리 (항목이 공용과 공유될 수 있으므로 전역 상태를 개인이 바꾸지 않는다).
- 관리자가 이미 개인 등록된 영상을 백오피스에서 등록하면 **공용 승격** (재추출 없음).
...
- CC 가 아니면(미확인 포함) **409 `cc_required`** — `content_permissions` 허락 증빙을
  첨부해야 진행 (기존 `allow_non_cc` 불리언 오버라이드는 2026-07-27 제거)
- 기존 콘텐츠 백필: `scripts/backfill_licenses.py`

### 담기(구독) 구조

같은 콘텐츠 1행을 여러 사용자가 공유하고 `content_subscriptions(content_id, user_id)` 로 연결한다.

- 사용자는 라이브러리에서 **담기**(`POST /api/my/contents/{id}/subscribe`, 멱등) → 재추출 없이 즉시 사용 (AI 비용 0)
- 담기 대상은 `ready` 인 공용 콘텐츠만. 준비 중이거나 개인 콘텐츠는 404
- "내 콘텐츠" 목록/상세/게임 소재/항목 노출 전부 담기 기준
...
문장 경계 시각 (2026-07-16, 라이브러리 3초 딜레이 수정): 경계 조각의 시작/끝을 그대로 쓰면 한 조각에 걸친 문장들이 구간을 3~5초씩 공유해 재생 문장 표시가 음성보다 늦어진다 — 두 규칙으로 보정한다.
- **문자 비례 보간**: 문장 경계 시각은 해당 조각 안의 문자 위치 비율로 추정
- **클램프**: 원본 조각끼리 겹치는 자막(2줄 롤링 표시)은 보간 후에도 경계가 겹침 — 시작 시각은 단조 증가로, 이전 문장 end 는 다음 문장 start 까지로 절단 (구간 겹침 0 보장)
- **크레딧 제거** (2026-08-05): 자막 선두의 "Translator:/Reviewer:/Transcriber: 이름" 이 발화와 병합돼 들어오면 세그먼트 저장 시 `strip_caption_credits` 로 제거 — 파이프라인·로컬 수집기 제출 양쪽 공통 (이름은 이름+성 2토큰, 문장부호에서 정지)

단어 정렬 (2026-07-22): 위 보간은 기본값(추정)이다. 로컬 에이전트가 오디오를 받아 단어별 시각을 채우면(`transcript_segments.words`) 문장 경계가 단어 시각에서 정확히 파생되고 단어 단위 반복/하이라이트가 가능해진다. 상세: [word-alignment.md](word-alignment.md).
