...
## 진입 경로 재설계 + 학습 중 알림 (2026-08-14 사용자 지시)

관전 기능이 있어도 /friends 배지 외 진입로가 없어 사장됨 — 발견 가능한
동선 3곳을 신설하고, 학습 시작을 접속 중 친구에게 실시간 알린다.

| # | 경로 | 동작 |
|---|---|---|
| 1 | **학습 중 토스트** | 친구가 관전 ON 으로 학습 시작 → 접속 중 친구에게 WS `st.friend_studying {user_id, nickname, code}` 릴레이 → InviteToaster 토스트 "OO 학습 중 — [관전]" (게임 초대 토스트와 동형). 종료 시 `st.friend_study_end` |
| 2 | **홈 대시보드 카드** | "지금 학습 중인 친구" — `GET /api/friends` 의 `studying`·`watch_code` 재사용, [관전] → `/study/watch?code=`. 학습 중 친구 없으면 카드 숨김 |
| 3 | **채팅에서** | 방 목록·대화방 헤더의 상대에 "학습 중" 배지 → 탭하면 관전 진입 (friends API lookup) |

- 알림은 **접속 중 채널만** (토스트) — 웹푸시 폴백 없음 (학습 시작은 빈번해
  푸시는 스팸. 게임 초대와 달리 명시적 초대가 아님).
- 프라이버시 불변: 관전 ON(opt-in) 인 학습자만 알림·배지 노출, 2중 동의
  (관전 수락) 유지. `SpectateHost` 토글은 **마지막 값 기억** (localStorage) —
  매번 다시 켜는 마찰 제거, 기본값은 여전히 OFF.
