...
- 단어 span 클릭 → `rangeRef = { start: (w.s-200)/1000, end: max(w.e+400, w.s-200+1200)/1000 }` 로 A-B 루프. 기존 루프 메커니즘 재사용.
- 구간 값의 근거 (유튜브 `seekTo` 계측, 2026-07-27): 착지 후 300~400ms 는 버퍼링이라 무음이고, 재생이 실제 시작되는 지점은 목표보다 약 200ms 뒤다. 즉 **단어 길이(300~500ms)보다 플레이어 오차가 크다.** 최초 구현의 `[w.s, w.s+400ms]` 창은 8초 측정에서 15회 재시킹하며 반복 1회당 단어의 32% 만 들렸다 → 선행 200ms(건너뛰기 보정) + 후행 400ms(끝음절) + 최소 1200ms 로 90% 까지 회복.
