...
구매 UI 를 설정에서 분리해 한 화면에: **XpWallet 지갑**(상단 고정, SHOP_EVENT
로 즉시 갱신) + 테마 섹션(`ThemeShopSection` — 가격·해금 힌트·구매, 사면 즉시
적용) + 캐릭터 섹션(`MascotShopSection` — 마스코트 카드·데려오기/쉬게 하기·
악세 칩·책갈피 충전, 부족 시 버튼에 부족분 표기) + 구매 내역(항상 펼침).
진입: 프로필 메뉴 "상점" + 설정의 바로가기 카드. 설정의 테마 섹션은 **전환
전용** — 잠긴 판매 테마는 "상점에서 N XP로 열기" 링크.
구매 실패 카피: XP 부족/이미 보유/책갈피 최대.

## 채팅 런처 대체 (2026-08-11)

활성 마스코트가 있으면 우하단 채팅 런처의 연필 아이콘 대신 **캐릭터가 버튼이
된다** (원본 방향·말풍선·idle 애니메이션 포함, 안읽음 배지 유지). 이때
`body.mascot-launcher` 로 좌하단 MascotPeek 를 숨겨 중복 노출을 막는다.
excel 위장 테마는 "메모" pill 유지 (캐릭터가 오피스 위장을 깬다), 도킹 모드는
런처가 없으므로 좌하단 유지.
