...
- **방 유지 재대결**: 방 게임(room) 정상 종료 시 방(세션·코드·플레이어)을 유지하고
  대기 상태로 복귀 — 방장 `qr.start` 가 새 매치 행을 발급해 재입장 없이 다음 판.
  시상대에 "같은 방에서 다시하기"(방장)/대기 안내(게스트). 솔로(봇 동반)·크래시 종료는 종전대로 정리
- **출제 히스토리**: 사용자별 최근 3판(30문항) 출제 단어를 제외하고 선정
  (`services/game/history.py ServedHistory` — 인메모리, 남는 단어가 한 판 분량 미만이면
  전체 풀 폴백). 의미 모드는 풀 필터, 뉘앙스 모드는 기록만. 규칙 원본: listening-bingo.md
