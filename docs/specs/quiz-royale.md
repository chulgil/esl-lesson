...
- 단어 풀 최소 15개 미만이면 `words_insufficient` — 테트리스와 **에러 코드는
  공유**하지만 최소 단어 임계값은 다름 (퀴즈로얄 15 / 테트리스 10, `MIN_POOL`
  vs `CONTENT_POOL_MIN`)
- **복습 우선 출제 (공유 원칙: 복습 우선 편입 — P0-A, 2026-08-10)**: 참가
  인원(사람)의 due 도래 + 최근 7일 오답 항목을 먼저 뽑고 나머지를 일반 풀에서
  채운다. 뉘앙스 모드는 제외
- **노출 방식(퀴즈로얄 고유)**: 우선 항목을 라운드 수만큼 먼저 선점한 뒤 전체
  출제 순서를 **다시 셔플** — 우선 항목이 앞라운드에 몰리지 않고 10라운드
  전반에 분산된다 (`build_questions`). 게임마다 노출 방식이 다르니 다른
  게임의 규칙을 그대로 가정하지 말 것 (테트리스는 큐 앞쪽 고정 배치).
- 이탈: 대기실=제거(호스트 이탈 시 방 해체), 진행 중=미제출 계속(재접속 대비
  매핑 유지), 인간 전원 이탈 시 진행 태스크 취소·세션 즉시 정리 — **결과
  미저장** (aborted 저장은 루프 오류 시에만, `detach`/`_finish`. 2026-08-13
  코드 대조로 정정 — 구 "중단 저장" 서술은 부정확)
...
- 패널은 `components/game/ReviewPanel.tsx` 공용 컴포넌트 (2026-07-16) —
  타자(`tp.review`)·어순(`sc.review`)·받아쓰기(`dt.review`)도 동일 규칙으로 사용
