...
- 시험지 표면은 테마별 스킨 (`theme-surfaces SURFACE_SKINS`, 학습 세션·OMR·퀴즈 선지·어순 칩과 한 벌 공용): 노트=종이 시험지 / 캔디=파스텔 사탕판 / 레고=블록판 / 헤냥이=크림 고양이 카드(발도장·살구 젤리) / 오피스=평가서 시트 / 학교수업=갱지 시험지 + **칠판 표제**(나무 프레임 + 딥그린 + 분필 글씨. 색은 게임 보드와 동일 값).
- **컨셉은 테마마다 배타적** (2026-08-04): 칠판은 학교수업만 쓴다 — 헤냥이가 칠판을 쓰고 있어 학습·시험 화면에서만 고양이 세계관이 끊기고 두 테마가 같아 보였다. 어두운 면은 **표제 상자에만** — 본문까지 칠판으로 덮으면 긴 영문 가독성이 떨어진다.
