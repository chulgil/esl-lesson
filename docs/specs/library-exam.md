...
> 2026-08-04 부분 갱신(코드 대조): 시험지 표면 스킨
...
- 시험지 표면은 테마별 스킨 (`theme-surfaces SURFACE_SKINS`, 학습 세션·OMR·퀴즈 선지·어순 칩과 한 벌 공용): 노트=종이 시험지 / 캔디=파스텔 사탕판 / 레고=블록판 / 헤냥이=크림 고양이 카드(발도장·살구 젤리) / 오피스=평가서 시트 / **학교수업=칠판**(나무 프레임 + 딥그린 판 + 분필 글씨, 답안지=출석부. 색은 게임 보드와 동일 값) / **학원=갱지 모의고사**(직각 인쇄물 + 이중 괘선 표제 + 빨간 채점펜 문항번호, 답안지=OMR 카드).
- **컨셉은 테마마다 배타적** (2026-08-04): 한 물체는 한 테마만. 칠판은 학교수업만 쓰고(헤냥이에서 회수), 갱지는 학원만 쓴다(학교수업에서 분리). 어두운 표면은 **문제 카드·답안지·게임 보드까지** — 앱 배경은 교실 벽(연민트)으로 남겨 장시간 학습 부담을 줄인다.
