...
| 8 | 기존 방 처리 | 마이그레이션으로 전 대화 **일반 방(plain) 백필** — 2026-08-14 사용자 결정: 설정 단계를 안 거친 기존 대화는 일반 대화. 학습 방은 마법사에서 명시적으로 생성 (초기 ko→en 학습 방 백필 결정을 대체) |
...
| 15 | 위장 테마와 언어쌍 표기 충돌 | 배지는 ASCII 텍스트("한→영")·소형 태그 — 말풍선·국기 이모지 금지 유지. 종류 접두어("학습"/"일반") 포함이 정본(LangPairBadge) — **전 표면 공용**: 전체 목록(노트·엑셀 표 언어 칼럼)·위젯 목록·방 헤더·엑셀 방 타이틀바 (2026-08-18: 엑셀 표·타이틀바 누락 보완) |
...
- 마이그레이션: 컬럼 추가(server_default) → 기존 유니크 제약 삭제 → 새 유니크 생성.
  기존 행(쌍의 유일한 방)은 **plain·friend·active** 로 백필 (2026-08-14 결정 #8).
...
- **기존 방 백필**: 개편 이전 방(= 쌍의 유일한 방)은 전부 plain 으로 전환
  (마이그레이션 f0a1b2c3d4e5). 레거시 전송 경로도 방이 하나도 없으면 plain 을
  만든다 — 학습 방은 마법사에서 명시적으로만 생긴다.
...
- **언어셋 불변 (2026-08-14 사용자 지시)**: 방의 source/target/mode 는 생성 후
  변경 불가 — 수정 API·UI 를 두지 않는다. 다른 언어쌍이 필요하면 새 방을
  만든다 (쌍+언어쌍+mode 유니크가 자연 유도). 수집·번역·게임 귀속이 방
  언어쌍을 근거로 하므로 사후 변경은 데이터 계보를 오염시킨다.
