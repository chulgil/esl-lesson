...
| 접속 중 — **"실제 대화 중"이 아니면 전부** | **OS 알림** (알림 켜기 수락 시). 대화 중 판정 = 그 방 표시 ∧ 탭 전면 ∧ 창 포커스 ∧ **입력창에 커서**(`data-chat-input` 마커 + `isChatInputFocused`). 방을 켜두고 커서가 밖이면 자리 비움으로 보고 알림 (2026-07-29 — 이전 조건 hidden∥!hasFocus 는 방 열린 포커스 탭에서 유실). 같은 tag 재알림은 `renotify`. 오피스 테마는 "공유 문서 / 변경 사항 1건" 으로 위장 |
...
- **여러 줄 입력** (2026-07-30, 2026-07-31 2차 개정): 공용 `ChatTextarea` — **기기별 Enter**: PC(sm 640px 이상, ChatWidget isDesktop 과 동일 기준) = Enter 전송·Shift+Enter 줄바꿈, 모바일 = Enter 줄바꿈·버튼 전송 (같은 날 '버튼 전용' → 'PC 는 Enter 전송' 재개정 — 데스크톱·모바일 관행이 달라 기기별 분기로 확정). 내용에 맞춰 자동 확장 후 **최대 8줄**에서 내부 스크롤, 전송 시 1줄 복귀. 세 입력면(NoteSkin/ExcelSkin/위젯) 공용, 입력줄은 items-end 로 버튼 바닥 고정. 말풍선은 `whitespace-pre-wrap` 으로 줄바꿈 보존
