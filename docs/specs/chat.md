...
| 접속 중 + 탭 백그라운드 | **OS 알림** (Notification API, 알림 켜기 수락 시) — 오피스 테마는 "공유 문서 / 변경 사항 1건" 으로 위장 |
...
| 테마 | 대화방 위장 | 목록 위장 |
|---|---|---|
| 노트·캔디·레고·헤냥이 | **교환 노트** — 필기 줄 형식, 내 글 파란 잉크/상대 검정 잉크, 컴팩트(max-w-md) | 교환 노트 목록 |
| 오피스(`excel`, 신규 전역 테마) | **스프레드시트** (excelkospi 컨셉) — 타이틀바(자동저장·`재고관리_날짜.xlsx`)·리본 탭·가짜 수식줄·행/열 그리드(시간\|담당\|내용\|상태)·시트 탭·상태바. 입력중 = "공동 작성자가 셀을 편집하는 중", 읽음 = 상태열 확인/미확인 | 공유 문서 목록 (파일명 = `{닉네임}_공유.xlsx`) |
...
## 플로팅 위젯 (2026-07-27 재설계)

excelkospi 우하단 버튼 컨셉 — 어느 화면에서든 작은 팝업(360×480)으로 대화.

- 런처: 우하단 고정. 오피스 = 초록 "메모" pill / 그 외 = 연필 아이콘 원형. 안읽음 배지 (WS 즉시 + 60초 폴링)
- 패널: 목록 뷰 ↔ 대화 뷰(useChatRoom 재사용). **Esc 한 번 = 즉시 닫기**
