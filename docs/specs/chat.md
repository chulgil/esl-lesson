...
> **2026-08-14 개편**: 대화방이 언어쌍 단위 학습 방으로 확장 — 방 모델
> (언어쌍 유니크·랜덤 매칭·origin/status)·방 기준 API·번역 반전 표시는
> [chat-language-rooms.md](chat-language-rooms.md) 가 정본. 이 문서의
> 대화 정규화(쌍당 1행)·`/chat/with/{id}` 계열은 레거시 위임 경로로 유지.
