...
| 접속 중 — **"실제 대화 중"이 아니면 전부** | 채널 분리 (2026-07-31 재설계): engaged(그 방 ∧ 전면 ∧ 입력 커서) = 무음 / **백그라운드**(hidden ∥ !hasFocus) = OS 알림(권한 없으면 토스트 폴백) / **전면인데 자리 비움**(커서 밖·다른 방·도킹 방치) = 인앱 토스트. 이전 설계는 viewing 이면 토스트를 막아 **도킹 모드 + OS 권한 없음 조합이 무알림**이었다. 같은 tag 재알림 `renotify`, 오피스 테마 위장 유지 |

- **알림 켜기**: 대화 목록의 `NotifyEnableButton` + **채팅 토스트의 [알림 켜기] 유도 버튼**(권한 default 일 때만, 2026-07-31 — 백그라운드에선 OS 알림이 유일 채널) — 기존 VAPID 푸시 구독(`subscribePush`) 재사용
...
- **파비콘 배지 + 탭 제목 카운트**: 안읽음 > 0 이면 파비콘 우상단 개수(99 한도) + **탭 제목 "(N) " 프리픽스** (2026-07-31 — OS 알림 권한 없이도 다른 탭에서 인지. 문서류 제목에 흔한 표기라 위장 유지). `lib/favicon-badge.ts`
...
- **여러 줄 입력** (2026-07-30, 2026-07-31 3차 최종): 공용 `ChatTextarea` — **Enter = 전송, Shift+Enter = 줄바꿈, 전 기기·전 화면(대화방/플로팅 위젯) 공통** (버튼 전용 → 기기별 분기 → 공통 확정: 기기별 분기는 터치스크린 노트북 오판·혼선만 낳아 폐기). IME 가드: isComposing + compositionend 후 30ms(사람 Enter >50ms 는 통과). 내용에 맞춰 자동 확장 후 **최대 8줄**에서 내부 스크롤, 전송 시 1줄 복귀. 세 입력면(NoteSkin/ExcelSkin/위젯) 공용, 입력줄은 items-end 로 버튼 바닥 고정. 말풍선은 `whitespace-pre-wrap` 으로 줄바꿈 보존
