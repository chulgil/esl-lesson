...
async def test_leave_on_result_screen_drops_player_and_blocks_ghost(wired_db, monkeypatch):  # noqa: F811
    ...
    host = await seed_user_and_words(wired_db)
    await seed_scramble_sentences(wired_db)
    stay = User(google_sub="g-sgh1", email="sgh1@example.com", name="ST")
    gone = User(google_sub="g-sgh2", email="sgh2@example.com", name="GO")
    ...
    await manager.leave(gone.id)  # 결과 화면 이탈 (sc.leave)
...
async def test_attach_sentence_includes_server_remaining(wired_db, monkeypatch):  # noqa: F811
    ...
    msgs = [m for m in resumed.messages if m["t"] == "sc.sentence"]
    assert msgs
    assert 0 < msgs[-1]["remaining"] < sc.SENTENCE_SECONDS
    manager._cleanup(session)
