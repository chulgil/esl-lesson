...
function StudySessionInner() {
  ...
  return (
    <main className="notebook-lines notebook-margin min-h-screen px-6 py-5 sm:px-16 sm:py-10">
      {/* 콘텐츠 열 중앙 정렬 — 전 화면 스윕과 통일 (2026-08-21 사용자 보고:
          오늘의 학습만 좌측 밀착으로 남아 있었다) */}
      <div className="mx-auto w-full max-w-2xl">
        ...
        {showSettings && (
          <div className="mb-6 flex max-w-xl flex-col gap-4 rounded-lg border-2 border-ink/15 bg-white p-4 text-sm">
            ...
            <label className="flex items-center gap-2 font-bold">
              ...
              />
              <span className="text-xs font-normal opacity-60">
                무응답이 이 시간을 넘으면 다음 한 단어를 힌트로 보여줘요
                (0=끄기)
              </span>
            </label>
            <button
        ...
        {(phase === "question" || phase === "feedback") && question && (
          <>
            ...
            {/* 문제↔정답 왕복 — 카드 박스 우측 상단 (2026-08-21 사용자 보고:
              카드 아래에 있으면 눈에 안 띈다 — 시선이 머무는 카드 상단으로) */}
            {phase === "feedback" && (
              <div className="mb-2 flex max-w-2xl flex-wrap items-center justify-end gap-2">
                {showFeedback ? (
                  <button
                    type="button"
                    onClick={() => setShowFeedback(false)}
                    className="inline-flex min-h-9 items-center gap-1 rounded-full border-2 border-ink/20 bg-white px-3 text-xs font-bold opacity-80 transition hover:border-ink/50 hover:opacity-100"
                  >
                    ← 문제 다시 보기
                  </button>
                ) : (
                  <>
                    ...
                    <button
                      ...
                      className="inline-flex min-h-9 items-center gap-1 rounded-full border-2 border-brick-blue/60 bg-white px-3 text-xs font-bold text-brick-blue transition hover:-translate-y-0.5 hover:border-brick-blue"
                    >
                      정답 확인 →
                    </button>
                  </>
            ...
            <div className="grid max-w-2xl overflow-x-clip">
              ...
              {phase === "feedback" && result && (
                <div
                  ...
                  inert={!showFeedback}
                >
                  {/* 판정 모션 — 정답=부드러운 안착 팝, 오답=미세 좌우 흔들림.
                      호 슬라이드(card-arc-in)와 transform 충돌을 피해 내부
                      래퍼에 건다. 회전 없음 — 어지럽지 않게 (2026-08-21 요청) */}
                  <div
                    className={
                      result.correct ? "card-grade-ok" : "card-grade-bad"
                    }
                  >
                    <SessionFeedback
                      question={question}
