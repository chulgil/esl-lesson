...
function ScrambleInner() {
  ...
  const startRound = useCallback((idx: number, remaining?: number) => {
    ...
    // 재접속 재전송이면 서버 기준 잔여로 — 전체 시간으로 되감으면 이미 마감된
    // 문장을 계속 조립하게 된다 (2026-08-20 교차 리뷰)
    deadlineRef.current = Date.now() + (remaining ?? secondsRef.current) * 1000;
    setPhase("racing");
  }, []);
    ...
    (msg: ServerMsg) => {
      switch (msg.t) {
        ...
        case "sc.start":
          ...
          break;
        case "sc.sentence":
          startRound(msg.idx, msg.remaining);
          break;
        case "sc.progress":
          setRivals((prev) => ({
            ...prev,
  ...
  useEffect(() => {
    return () => {
      if (leaveRoomOnUnmountRef.current) socketRef.current?.scLeave();
