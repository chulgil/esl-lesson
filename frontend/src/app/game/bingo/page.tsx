...
function BingoInner() {
  ...
  const handleMessage = useCallback((msg: ServerMsg) => {
    switch (msg.t) {
      ...
      case "bg.start":
        ...
        setEnd(null);
        setReview([]);
        // 재접속이면 서버가 채운 칸·상대 진행을 복원해준다 (2026-08-20 튕김 픽스)
        setFilled(new Set(msg.filled ?? []));
        setMarks(msg.marks ?? {});
        setRound(null);
        setReveal(null);
        setError(null);
  ...
  useEffect(() => {
    const socket = new GameSocket(
      handleMessage,
      () => {
        // 자동 재접속 중 — 서버 attach 가 진행 상태를 복원한다 (게이트: 새로고침 불필요)
    ...
    // 첫 탭/클릭에서 오디오 채널 해금 — 서버 트리거 자동 재생(bg.round)이
    // 브라우저 정책에 막히지 않게 (2026-08-20 자동재생 보고)
